    ...
    <div className="rounded-md shadow-md p-8 border-[1px] border-gray-300">
      <div className="flex items-center justify-between mb-2">
        ...
        <p className="text-md max-md:text-sm">
          {formatTimeDifference(props.created_at)}
        </p>
      </div>
      <div className="ms-10">
        <div className="mb-8">
        ...
        <div className="flex flex-grow gap-x-2 mb-2">
          {props.tags &&
            props.tags.map((value, index) => (
              <div
                key={index}
                className="text-sm border-[1px] border-gray-300 rounded-md p-2 max-md:text-xs"
              >
                {value.name}
              </div>
            ))}
